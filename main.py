...
async def create_match_embedded(match_ambig):
    ...
    embed = discord.Embed(title="Match:",
                          color=discord.Color.from_rgb(*tuple(
                              int((match.code[0:8])[i:i + 2], 16)
                              for i in (0, 2, 4))))

    embed.add_field(name="Teams:",
                    value=match.t1 + " vs " + match.t2,
                    inline=True)
    embed.add_field(name="Odds:",
                    value=str(match.t1o) + " / " + str(match.t2o),
                    inline=True)
    ...
    if match.date_closed == None:
        ...
        closed_date_formatted = match.date_closed.strftime(
            "%m/%d/%Y at %H:%M:%S")
        embed.add_field(name="Betting Closed:",
                        value=closed_date_formatted,
                        inline=True)
...
async def create_bet_list_embedded(embed_title, bets_ambig):
    ...
    if all(isinstance(s, str) for s in bets_ambig):
        for bet_id in bets_ambig:
            ...
            embed.add_field(name="\n" + "Bet: " + bet.code,
                            value=await bet.short_to_string(bot) + "\n",
                            inline=False)
    else:
...
async def create_user_embedded(user_ambig):
    ...
    embed.add_field(name="Balance:",
                    ...
                    inline=False)
    embed.add_field(name="Balance Available:",
                    value=math.floor(user.balance[-1][1] -
                                     get_user_unavailable_balance(user)),
                    inline=False)
    return embed
...
def add_balance_user(user_ambig, change, description):
    ...
    user.balance.append((description, round(user.balance[-1][1] + change,
                                            5), datetime.now()))
    replace_in_list("user", user.code, user)
    return user
...
async def on_message(message):
    ...
    if message.content == "$clear the database of bad keys please and thank you":
        ...
        db["log_" + get_uniqe_code("log")] = jsonpickle.encode(
            ("a little called " + message.author.name + " ID: " +
             str(message.author.id) + " cleared database \nkeys include " +
             str(keys)), datetime.now())
        ...
        if stage >= 0:
            if stage == 1:
                ...
                if message.author.id == db["current_user"]:
                    ...
                    stemp = s.replace('.', '')
                    if (is_digit(stemp)
                            and (s.find('.') == -1 or s.find('.') == 1)):
                        await message.channel.send(
                            "What is Team 2's Odds\nEnter in the amount you would get if you bet 1"
                        )
                ...
                if message.author.id == db["current_user"]:
                    ...
                    if (is_digit(stemp)
                            and (s.find('.') == -1 or s.find('.') == 1)):
                        await message.channel.send(
                            "Do you want to balance the odds?")
                ...
                if message.author.id == db["current_user"]:
                    ...
                    if message.content.lower() == "yes":
                        ...
                        await message.channel.send(
                            "The new odds are " + str(db["current_t1_odds"]) +
                            " / " + str(db["current_t2_odds"]) +
                ...
                if message.author.id == db["current_user"]:
                    ...
                    cmatch = Match(t1n, t2n, t1oo, t2oo, t1o, t2o, tn,
                                   odds_source, message.author.id,
                                   datetime.today(), code)
                    ...
                    s = "Is this right?\n"
                    date_formatted = cmatch.date_created.strftime(
                        "%m/%d/%Y %H:%M:%S")
                    s += "Teams: " + str(cmatch.t1) + " vs " + str(
                        cmatch.t2
                    ) + "\nOdds: " + str(cmatch.t1o) + " / " + str(
                        cmatch.t2o) + "\nTournament Name: " + str(
                ...
                if message.author.id == db["current_user"]:
                    ...
                    if message.content.lower() == "yes":
                        ...
                        msg = await (await
                                     bot.fetch_channel(db["match_channel_id"]
                                                       )).send(embed=embedd)
...
async def match(ctx, *args):
    ...
    if len(args) == 0:
        ...
        if args[0] == "help":
            await ctx.send("""$match: starts match creation
...
$match list full: sends embed of all matches without a winner""")
            ...
            await ctx.send(
                "Not valid command. Use $match help to get list of commands")
        ...
        if args[0] == "delete" and len(args[1]) == 8:
            ...
            for match in matches:
                if int(match.winner) == 0 and (set(
                        user.active_bet_ids).isdisjoint(match.bet_ids)):
                    match_list.append(match)
            if len(match_list) == 0:
                await ctx.send("No undecided matches.")
                return
        ...
        if args[0] == "close" and args[1] == "betting" and len(args[2]) == 8:
        ...
        elif args[0].startswith("winner") and len(args[1]) == 8 and len(
                args[2]) == 1 and is_digit(args[2]):
            match = get_from_list("match", args[1])
            if match == None:
                await ctx.send("Identifier Not Found")
                return
            if int(match.winner) == 0 or args[0] == "winnerforce":
                ...
                no_same_list_user = []
                [
                    no_same_list_user.append(x) for x in users
                    if x not in no_same_list_user
                ]
...
$bet winner [bet id]: sets the bets winner (should mostly only be used after an error)"""
            ...
            await ctx.send(
                "Not valid command. Use $bet help to get list of commands")
        ...
        if args[0].startswith("winner") and len(args[1]) == 8 and len(
                args[2]) == 1 and is_digit(args[2]):
            bet = get_from_list("bet", args[1])
            if bet == None:
                await ctx.send("Identifier Not Found")
        ...
        balance_left = user.balance[-1][1] - int(
            amount) - get_user_unavailable_balance(user)
        if balance_left < 0:
            await ctx.send("You have bet " + str(math.floor(-balance_left)) +
                           " more than you have")
            return

        bet = Bet(code, match_id, ctx.author.id, int(amount), int(team_num),
                  datetime.now())
        ...
        embedd = await create_bet_embedded(bet)
        msg = await (await bot.fetch_channel(db["bet_channel_id"]
                                             )).send(embed=embedd)

        await edit_all_messages(bet.message_ids, embedd)
        bet.message_ids.append((msg.id, msg.channel.id))
        replace_in_list("bet", bet.code, bet)
        if ctx.channel.id == db["bet_channel_id"] or ctx.channel.id == db[
                "match_channel_id"]:
            await ctx.message.delete()
...
$debug balance rename [user @] "old balance id" "new balance id": replaces balance ID with new ID"""
            ...
            await ctx.send(
                "Not valid command. Use $debug help to get list of commands")

    elif len(args) == 2:

        if args[0] == "list" and (args[1] == "match" or args[1] == "user"
                                  or args[1] == "bet"):
...
async def assign(ctx, *args):
    if not len(args) == 1:
        await ctx.send(
            "Not a valid command do $assign help for list of commands")
        return
    arg = args[0]
    if arg == "help":
        await ctx.send("""$assign creation: where match creation takes place
...
async def balance(ctx, *args):
    if len(args) == 0:
        ...
        uid = uid.replace("!", "")
        if args[0] == "help":
            await ctx.send("""$balance gives your own balance
$balance [user's @]: gives balance of that user""")
            ...
            await ctx.send(
                "Not a valid command do $balance help for list of commands")
    else:
        await ctx.send(
            "Not a valid command do $balance help for list of commands")
...
async def award(ctx, *args):
    if len(args) == 1:
        ...
        if not (is_digit(uid) and is_digit(args[1])):
            await ctx.send(
                "Not a valid command do $award help for list of commands")
            return
...
async def leaderboard(ctx, *arg):
    ...
    if len(arg) == 0:
        ...
        if len(arg) == "help":
            ...
            await ctx.send(
                "Not a valid command do $leaderboard help for list of commands"
            )
    else:
        await ctx.send(
            "Not a valid command do $leaderboard help for list of commands")
...
async def round_all_user_balances(ctx):
    ...
    for user in users:
        for bal in user.balance:
            ...
            user.balance[user.balance.index(bal)] = (
                user.balance[user.balance.index(bal)][0],
                round(user.balance[user.balance.index(bal)][1],
                      5), user.balance[user.balance.index(bal)][2])
    ...
    for user in users:
        ...
        for bal in user.balance:
            if len(user.balance[user.balance.index(bal)][0]) == 8:
                user.balance[user.balance.index(bal)] = (
                    ("id_" + str(user.balance[user.balance.index(bal)][0])),
                    user.balance[user.balance.index(bal)][1],
                    user.balance[user.balance.index(bal)][2])
